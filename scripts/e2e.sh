...
# `watch-store --drain` pushes the rest. Here the daemon runs against a Pusher
# that cannot be reached, so its push fails and the path is the drain's.
sed -e 's/watcher-cursor/drain-cursor/' "$root/client.toml" > "$root/client-drain.toml"
...
"$bin"/garret --config "$root/client-drain-dead.toml" watch-store > "$root/drain-watch.log" 2>&1 &
drain_watch_pid=$!
...
# The daemon's half: a failed push goes on the failed list rather than being
# lost behind the cursor. Its retries against the dead Pusher take ~15 s.
for _ in $(seq 600); do
  grep -qx "$drained" "$root/drain-cursor.failed" 2>/dev/null && break
  sleep 0.1
done
grep -qx "$drained" "$root/drain-cursor.failed" || {
  echo "the daemon's failed push is not on the failed list"; cat "$root/drain-watch.log"; exit 1; }
kill $drain_watch_pid
wait $drain_watch_pid 2>/dev/null || true
# A drain that still cannot reach the Pusher fails and keeps the path.
